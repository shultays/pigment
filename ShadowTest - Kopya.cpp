...
//D3DXVECTOR3 ShadowTest::m_pVertices[32000];
//int ShadowTest::m_dwNumVertices = 0;

ShadowTest* ShadowTest::allShadows[MAX_SHADOW];
int ShadowTest::shadowSize = 0;

ID3DXEffect* ShadowTest::shadowEffect;

void ShadowTest::addShadowList(){
...
void ShadowTest::calculateShadow(D3DXVECTOR3 light){
  ...
  for( DWORD i=0; i<dwNumFaces; i++ ){
    ...
    if( D3DXVec3Dot( &Tools::Normalize(vNormal), &Tools::Normalize(center-light) ) < -0.1f ){
        
      m_pVertices[m_dwNumVertices++] = v0;
      m_pVertices[m_dwNumVertices++] = v2;
      m_pVertices[m_dwNumVertices++] = v1;

      m_pVertices[m_dwNumVertices++] = v0 - Tools::Normalize(light-v0)*500;
      m_pVertices[m_dwNumVertices++] = v1 - Tools::Normalize(light-v1)*500;
      m_pVertices[m_dwNumVertices++] = v2 - Tools::Normalize(light-v2)*500;
  ...
  for(DWORD i=0; i<dwNumEdges; i++ ){		
    ...
    D3DXVECTOR3 v3 = v1 - Tools::Normalize(light-v1)*500;
    D3DXVECTOR3 v4 = v2 - Tools::Normalize(light-v2)*500;
...
void ShadowTest::drawShadowVolume(){

  Tools::D3DDevice->SetFVF( D3DFVF_XYZ );

  D3DXMATRIX W1, W2;
  D3DXMatrixScaling(&W1,size, size, size);
  D3DXMatrixTranslation(&W2, coors.x, coors.y, coors.z);
  W1 = W1*W2;

  shadowEffect->SetMatrix( "World", &(W1));
	shadowEffect->SetMatrix( "WorldViewProjection", &(W1 * (*View) * (*Proj)) );
...
HRESULT GenerateShadowMesh( IDirect3DDevice9* pd3dDevice, ID3DXMesh* pMesh, ID3DXMesh** ppOutMesh )
{
    ...
    *ppOutMesh = NULL;
    ...
    Tools::g->con.buff <<      
      "Input mesh has %u vertices, %u faces\n"
      <<
      pInputMesh->GetNumVertices()
      <<pInputMesh->GetNumFaces() ;
...
ShadowTest::ShadowTest(char *xFile): BasicX (xFile){
  
  GenerateShadowMesh(Tools::D3DDevice, mesh, &shadowMesh);
