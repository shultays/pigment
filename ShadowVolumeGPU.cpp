...
ID3DXEffect* ShadowVolumeGPU::shadowEffect;

#define ADJACENCY_EPSILON 0.0001f

struct MESHVERTEX { D3DXVECTOR3 p, n; FLOAT tu, tv; };

void ShadowVolumeGPU::drawShadowVolume(D3DXVECTOR3 light){

  
    shadowEffect->SetVector( "g_vLight", &D3DXVECTOR4(light, 1.0));

    shadowEffect->SetMatrix( "g_mWorld", &(base->World)) ;
    shadowEffect->SetMatrix( "g_mViewProjection", &(*base->View * *base->Proj)) ;
  

    shadowEffect->SetTechnique( "RenderShadowVolume2Sided" );
    IDirect3DVertexDeclaration9*    g_pShadowDecl;
    ...
    Tools::D3DDevice->CreateVertexDeclaration( Decl, &g_pShadowDecl );

    Tools::D3DDevice->SetVertexDeclaration( g_pShadowDecl ) ;
    UINT cPasses;

                      
    shadowEffect->Begin( &cPasses, 0 ) ;
...
ShadowVolumeGPU::ShadowVolumeGPU(BasicX *b){
  
  base = b;
  
  GenerateShadowMesh(Tools::D3DDevice, b->mesh, &shadowMesh);
}
